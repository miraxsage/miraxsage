...
interface LanguagesByCommitsProps {
    languages: { name: string; color: string | null; commits: number }[];
}

export default function LanguagesByCommits({ languages }: LanguagesByCommitsProps) {
    const theme = useTheme();
    const labelColor = getThemeColor("regularText", theme);

    const top5 = languages.slice(0, 5);
    const total = top5.reduce((sum, l) => sum + l.commits, 0);
